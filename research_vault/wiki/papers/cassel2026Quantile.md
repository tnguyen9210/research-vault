...
**Verification flag (2026-09-22): the bias bound is false as stated.** Its proof in §A.1 applies the corollary printed under Lemma 14 as $`2\sqrt{(e-2)\sigma^2T}+R\log(1/\delta)`$. The $`\lambda`$ stated there actually gives $`2\sqrt{(e-2)\sigma^2T\log(1/\delta)}+R\log(1/\delta)`$, and the source, Beygelzimer et al. (2011, Theorem 1), has the logarithm under the root. At the proof's per-batch $`\delta=4.7\times10^{-4}`$, $`2\sqrt{e-2}=1.695`$ is exactly the $`1.7`$ and $`\log(1/\delta)=7.66`$ is the $`8R`$. The route supports a coefficient of about $`4.7`$. For Bernoulli rewards with parameter $`1/2`$ and $`B\ge26\ln(1/\delta)`$, the stated event holds with probability tending to at most $`\delta^{0.345}`$ as $`n\to\infty`$. A proved replacement, with $`4`$ and $`\tfrac{11}{3}RB/n`$ in place of $`1.7`$ and $`9RB/n`$, is Lemma 4 of [[2026-09-22-qom-value-based-pessimism]], which also gives the counterexample. Rates, including Theorem 4's, are unaffected; the constants $`22`$ and $`1924`$ were not re-derived.
