...
**Definition:** Learning a policy entirely from a fixed dataset $\mathcal{D}$ collected by some behavior policy $\pi_\beta$, with no further environment interaction. The binding constraint is not optimization but **distributional shift**: improving over $\pi_\beta$ requires knowing the value of actions $\pi_\beta$ did not take.
...
An MDP $(\mathcal{S}, \mathcal{A}, P, r, H)$ with a fixed dataset $\mathcal{D} = \{(s, a, r, s')\}$ of transitions generated by $\pi_\beta$. The learner outputs $\hat\pi$ without further interaction, and is judged by the suboptimality

$$\Delta(\hat\pi) = J(\pi^*) - J(\hat\pi),$$

where $J(\pi) = \mathbb{E}_\pi\big[\sum_{h=1}^H r_h\big]$. Since $\pi^*$ may visit states and actions $\mathcal{D}$ never contains, $\Delta(\hat\pi)$ is not controllable without an assumption relating $\mathcal{D}$'s distribution to $\pi^*$'s. That assumption is a **coverage** condition, and which one is assumed is the single largest axis of variation in the theory: uniform coverage bounds the ratio for *every* policy, single-policy coverage only for $\pi^*$, and the latter is what [[pessimism-principle]] buys. See [[coverage-coefficient]].
...
**Policy-constraint methods** (BCQ, BEAR, AWAC, TD3+BC) restrict how far $\hat\pi$ may deviate from $\pi_\beta$. **Value-regularization methods** (CQL, Fisher-BRC) leave the policy free but push $Q$ down on out-of-distribution actions. Both impose an explicit improvement-versus-safety trade-off, tuned by a coefficient. **In-sample methods** ([[implicit-q-learning]]) never query an unseen action at all: IQL fits an upper [[expectile-regression]] of $Q(s,\cdot)$ over the actions actually present, then extracts a policy by advantage-weighted regression. That dissolves the trade-off during value learning rather than balancing it — at the cost of targeting only the support-constrained optimum, which is weaker than $\pi^*$ whenever $\pi_\beta$ misses good actions entirely.
...
- **Finite-$\tau$, finite-sample guarantees for in-sample methods.** IQL's Theorem 3 is asymptotic in $\tau$ and assumes exact solutions. No bound exists on $\max_{a:\pi_\beta(a|s)>0} Q^*(s,a) - V_\tau(s)$ at the $\tau$ values used in practice. [[Yin2023Offline]] shows what such a bound looks like for the pessimism family; transporting it is the obvious target.
...
- **Instance-dependent dataset quality.** Support ($\pi_\beta(a|s) > 0$) is binary and ignores how much mass sits near the maximizing action. A [[coverage-coefficient]]-style density-weighted quantity should govern both the achievable value and the required $\tau$, and would explain why $\tau = 0.9$ is necessary on antmaze but not on locomotion.
