...
| Coverage in the bound | $`C^{\pi^*_\Pi}_2`$ (Bernstein widths); $`C_\gamma(\pi^*_\Pi)`$ (IX) | $`C^*`$, $`\bar C^*`$, $`C_{\mathcal F}(\pi^*)`$, $`\mathbb E_{\pi^*}\Vert\phi\Vert_{\Lambda^{-1}}`$ (§4) |
...
$`\mu_1-\mu_J\le W^L_1+W^U_J`$ and PES giving $`\mu_1-\mu_J\le W^L_1+W^U_1`$. Read over policies rather than indices these are the
