...
### Mean reward, value, and optimal policy
...
### The value-based route, in one paragraph
...
### The policy-based route, in one paragraph
...
**Proposition 1 (Dudík, Langford & Li 2011, Theorems 1–2, bandit form).** Let $g:=\hat q-q^*$, $\delta_\mu:=1-\mu/\hat\mu$, $g(x,\pi):=\sum_a\pi(a\mid x)g(x,a)$. Conditional on $\hat q$ and $\hat\mu$,
$$
...
### Class-restricted learning with imputed rewards
...
The policy-based route of the policy-based route above is estimate-then-select with importance-weighted scores. Its guarantees are the same two inequalities as the two suboptimality lemmas in [[value-based-offline-bandits]], read over policies.

**Proposition 2 (MaxIPW and PES).** Let $\Pi$ be finite and $W_\pi:=\frac1{\mu_{\min}}\sqrt{\ln(2|\Pi|/\delta)/(2T)}$, so that $|\widehat J^{\mathrm{IPW}}(\pi)-J(\pi)|\le W_\pi$ for all $\pi\in\Pi$ with probability at least $1-\delta$ (Hoeffding, as in [[value-based-offline-bandits]]; summands in $[0,1/\mu_{\min}]$). Then MaxIPW satisfies $\Delta_\Pi\le W_{\pi^*_\Pi}+W_{\hat\pi}$ and PES with any valid per-policy widths satisfies $\Delta_\Pi\le W^L_{\pi^*_\Pi}+W^U_{\pi^*_\Pi}$.
...
| Source of bias | none (IPW); chosen, $\gamma C_\gamma(\pi)$ (IX) | misspecification, $\mathrm{dist}(q^*,\mathcal F)$; does not vanish with $T$ (Prop. 6.4) |
...
| Deficient support | fatal: weights undefined | tolerated through $C_{\mathcal F}$ (the change-of-measure lemma in [[value-based-offline-bandits]](c)) |
| Comparator | best in $\Pi$ | global optimum $\pi^*$ |
| Uncertainty quantified | per policy, $W_\pi$ | per pair, $b(x,a)$ / $\Gamma(x,a)$ (Def. 3.10) |
| Pessimism | $\arg\max_\pi\widehat J(\pi)-W^U_\pi$ | $\arg\max_a\hat q(x,a)-\Gamma(x,a)$; equals per-policy pessimism on $\mathcal A^{\mathcal X}$ (Prop. 7.1) |
...
| Fast rates | $1/T$ only under a gap in policy values | $1/T$ under per-context action gaps (Thm. 8.2) |
| Minimax OPE | IPS/DR match the lower bound | DM alone does not; DR combines both (the doubly robust estimator above) |
