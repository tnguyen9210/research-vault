...
## [2026-09-22] query | Quantile of Means pessimism, the sub-Gaussian case → [[2026-09-22-qom-value-based-pessimism-subgaussian]]

- Tuan asked for the analysis of [[2026-09-22-qom-value-based-pessimism]] simplified to the standard bandit setting, as a separate page. Settled over several rounds: σ-sub-Gaussian noise with one common σ, every pair logged at least B times (C), α = 1/8, and the algorithm in the listing form the offline-CB literature uses
- **Sub-Gaussianity alone does not make the quantile pessimistic.** A tail bound gives the width but not how often a batch mean undershoots. A two-point σ-sub-Gaussian law puts the estimate σ/8 above the mean with probability at least 1/2 for every B, and on a two-action instance Theorem 1's bound tends to 0 while the regret does not (Proposition 3). One assumption added, (U): a batch mean is at most its mean with probability at least 1/2, which symmetric noise gives (Fact 1, proved)
- **Results.** Theorem 1: width 4σ√(ln2 / ⌊N/B⌋) charged once, B = 3.2 ln(2SK/δ), no B/N term, no clip. Theorem 2: 8σ√(ln2 · S C̄* B / T) — homoscedasticity collapses the proxy-weighted coverage to σ²C̄*, so the bound is Theorem 4.2's own expression in its own quantities and Corollary 1's constant comparison is exact: 5.96 against 2 for rewards in [0,1], and 4.2 against the exact-σ Hoeffding LCB. Exact recovery at noiseless optimal actions. Theorem 3 gives the uniform-coverage bound that survives without (U); the (C*−1) loss persists
- **Two things recorded rather than glossed.** (C) is a uniform-coverage condition that excludes expert data, so Corollary 3 is stated without it, and §7.1 shows the proofs only ever need it at π*'s actions. And homoscedasticity makes the variance adaptivity invisible; a remark after Theorem 2 recovers the V*_σ forms, since no lemma uses a common proxy
- **Structure, per Tuan:** main results first, supporting lemmas after, each theorem proved step by step in the main text in the style of the Overleaf note's Theorem 1. The one-sided reduction and the Chernoff count step are now inline in Theorems 1 and 2; only the per-cell facts remain lemmas
- Every tool cited and its hypotheses checked at the point of use, read from the mirrored PDFs: Lattimore & Szepesvári Def. 5.2, Thm 5.3, Lemma 5.4, Cor. 5.5, Ex. 5.6; Boucheron–Lugosi–Massart Lemma 2.2. The kl constants carry checks by hand. 0 of 734 spans wrong on GitHub's renderer, MathJax 0 errors
- A reversed inline-math delimiter in one proof broke 314 spans downstream; the stray-`$` rule added to `check_math.py` this morning caught it. The general page links here from its scope line; index entry added

## [2026-09-22] update | QoM written into the Overleaf note, and the linear case worked out → [[2026-09-22-qom-value-based-pessimism-subgaussian]]

- Theorems 1 and 2 and the core lemmas are now Overleaf §2.5 of `02_offline_contextual_bandits.tex`, in that note's own notation and proof style: setting and algorithm, assumptions, the two theorems with step-by-step proofs, lemmas last. Each proof step opens with what it is meant to achieve. Tuan then trimmed the commentary, so §2.5 is now the bare mathematics
- **§2.6 is new mathematics, not a transcription.** The linear version splits the rounds rather than each cell's rewards and fits one least-squares estimate per batch. The enabling fact: a least-squares prediction is a noise average with *signed* weights, so non-negativity of the rewards is useless, but symmetry does not care about signs — a sum of independent symmetric variables is symmetric whatever the coefficients. That supplies exactly the "new undershoot lemma" the general page listed as open, and the general page now says so
- The width carries over with ‖φ‖ in (Λ^b)^{-1} replacing 1/√⌊N/B⌋, the cross terms telescoping against Λ^b, and the same 1/256 per batch. Theorem: 4σ√(ln2)·E max_b ‖φ(x,π(x))‖ in (Λ^b)^{-1}, charged once. Steps 2 and 3 of the tabular proof are used verbatim — they never touch the tabular structure. At φ = e_(x,a) the bound collapses to Theorem 1 exactly
- **Two limitations kept on the record rather than buried.** The fit must be ordinary least squares: ridge shrinkage is a deterministic bias inside the order statistic and would break the symmetry, so Λ^b ≻ 0 replaces the regularizer. And infinite context sets are not covered — the union bound is over the SK pairs; the covering route is sketched with the Lipschitz constant it needs and the B ≳ d ln T it forces
- The two load-bearing identities were checked numerically on random designs before writing: Σ c_t² = ‖φ‖² in (Λ^b)^{-1} to machine precision, the deviation identity, the tabular collapse, and t = 4σ√(ln2)‖φ‖ giving exactly 2^{-8}
- Overleaf commits `71360a6` through `9a66089`. The Overleaf PDF was not rebuilt, per Tuan's standing instruction

## [2026-09-22] query | Cassel & Rosenberg's Lemma 1 and Corollary 2, checked → [[2026-09-22-cassel-qom-lemma1-erratum]]

- Tuan decided to build the Overleaf tabular section on [[cassel2026Quantile]] §2 directly — their estimator definition, Lemma 1 and Corollary 2 — rather than on the versions reproved in the two QoM query pages, and asked what exactly is wrong with those two results. The answer is now its own page, since it was scattered across §7.1 of the general page and a flag on the paper page
- **Corollary 2 holds**, from Feige (2004) Theorem 1, with one corner case: the proof divides by μ, and at μ = 0 the strict inequality it states is actually false, since a non-negative variable of mean 0 is identically 0. The non-strict form holds everywhere and is the one to use offline, where an action that never pays is an ordinary cell
- **Lemma 1(1), optimism, holds** — and it is the half that matters, being what replaces an uncertainty bonus. kl(1/65, 1/13) = 0.038785 against the 1/26 = 0.038462 the argument needs
- **Lemma 1(2), the bias bound, is false.** Their Freedman corollary instantiates λ at its minimizer but reports 2√((e−2)σ²T) where substituting that λ gives 2√((e−2)σ²T·log(1/δ)); the log was dropped from under the radical, and Beygelzimer et al.'s Theorem 1 carries it inside. Two numbers show it is a transcription slip: 2√(e−2) = 1.69503 is their 1.7, and their second term's 9R matches R·log(1/δ₀) = 7.64R at the δ₀ = 4.83e−4 their Binomial step needs. The constant their own route supports is 4.68
