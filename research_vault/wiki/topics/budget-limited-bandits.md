...
2. **Pure-exploration / fixed-confidence cost minimization** — identify the best arm with confidence $1-\delta$ while minimizing *cumulative cost*. This is cost-aware [[bai]].
...
- [[TranThanh2010Epsilon]] (2010) — **introduces the budget-limited MAB**, its unbounded-knapsack optimum, and the reward-density statistic; solves it with an [[budget-limited-mab-epsilon-first]] policy (uniform exploration on $\varepsilon B$, density-ordered greedy on the rest) and proves the first loss bound. Optimizing that bound over $\varepsilon$ gives $O(B^{2/3})$
- [[TranThanh2012Knapsack]] (2012) — [[budget-limited-mab-kube]] / fractional KUBE; first $O(\ln B)$ regret with matching lower bound, by interleaving exploration into the knapsack objective instead of giving it a separate phase
...
- [[bai]] — the pure-exploration regime that the cost-aware BAI thread specializes
