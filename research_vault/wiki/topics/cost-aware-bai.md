...
## Overview
The literature extending [[bai]] (see the concept page for the paradigm, lower bound, and TAS) with **heterogeneous arm costs** — the vault's most developed pure-exploration thread. Cost enters in fundamentally different roles across the papers: as the objective to minimize ([[bai-cost-aware]], where the optimal allocation shifts to $\sqrt{c_a}$ proportions), as a feasibility constraint ([[bai-constrained]]), and under a fixed budget with multiple stochastic constraints (BFAI). The paradigm's mature machinery (transportation-cost lower bounds, Track-and-Stop) carries over, but each cost role reshapes the optimal allocation differently.
